...
float dist ( Point p1, Point p2, int dim )
{
	...
	//numofelements = omp_get_thread_num();
	
	#pragma omp simd  reduction(+:temp_result)  
	for ( i=0; i<dim; i++ )
		temp_result += ( p1.coord[i] - p2.coord[i] ) * ( p1.coord[i] - p2.coord[i] );
...
{
	int i, tid , numofthreads , temp_assign; //assign : i may not need temp,check later
	long start , end , chunk;
	int number_of_centers_to_close = 0;
	//float x_cost, current_cost ;
	float temp_x_cost , temp_current_cost;
	...
	double temp_cost_of_opening_x = 0;
	...
	//double* temp_lower = &work_mem[0];
	//global *lower* fields
	double* gl_lower = &work_mem[stride];


#pragma omp parallel default(shared) private(start , end , tid , numofthreads , chunk  ) reduction(+: temp_cost_of_opening_x)
{
tid = omp_get_thread_num();
numofthreads = omp_get_num_threads();
chunk = points->num / numofthreads;
start = chunk * tid;


if(tid == 0){
end = start + chunk - 1 ;
}else if(tid == (numofthreads - 1) ){
end = points->num;
}else{
end = start + chunk;
}
	...
	for ( i = start; i < end; i++ ) {
		temp_x_cost = dist ( points->p[i], points->p[x], points->dim ) * points->p[i].weight;
		temp_current_cost = points->p[i].cost;

		if ( temp_x_cost < temp_current_cost ) {
			...
			temp_cost_of_opening_x += temp_x_cost - temp_current_cost;
			...
			temp_assign = points->p[i].assign;
			lower[center_table[temp_assign]] += temp_current_cost - temp_x_cost;
		}
	}
}
cost_of_opening_x = temp_cost_of_opening_x;
...
float pkmedian ( Points *points, long kmin, long kmax, long* kfinal )
{
	...
	hiz = loz = temp_hiz = 0.0;
	long ptDimension = points->dim;
	numofthreads = omp_get_num_threads();
	chunk = points->num / numofthreads;
	
#pragma omp parallel for schedule(dynamic , chunk) reduction(+:temp_hiz) private(kk)
	for ( kk = 0; kk < points->num; kk++ )
	temp_hiz += dist ( points->p[kk], points->p[0], ptDimension ) * points->p[kk].weight;
